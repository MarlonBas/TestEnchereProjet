...
<body>
	...
	<form action="Recherche" method="get" class="searchBar">
		...
		<label for="categorie">Catégorie :</label> <select id="categorie"
			...
			%>
		</select> <input type="submit" value="Recherche" class="button" />
	</form>
	...
	<div class="listeEncheres">
		...
		%>
		<div class="article">
			<a class="titreArticle"
				href="AfficherArticleVendu?id_article=<%=article.getNoArticle()%>"><%=article.getNomArticle()%></a>
			<p>
				Prix actuel :
	...
	while (iterator2.hasNext()) {
		...
		if (enchere.getUtilisateur().getPseudo().equals(utilisateur.getPseudo())) {
	...
	while (iterator3.hasNext()) {
		...
		if (enchere.getUtilisateur().getPseudo().equals(utilisateur.getPseudo())) {
			...
			&& enchere.getArticleVendu().getPrixVente() == enchere.getMontantEnchere())
